    ...
    <div class="max-w-5xl mx-auto">
      ...
      <div class="grid grid-cols-1 gap-6">
        <div v-for="task in tasks" :key="task.id"
          class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 hover:shadow-md transition-all">
          <div class="flex flex-col md:flex-row justify-between gap-4">
            <div class="flex-1">
              <div class="flex items-center gap-2 mb-2">
                <span :class="getStatusInfo(task.status).class"
              ...
              <div class="mt-4">
                <div v-if="task.comments && task.comments.length > 0" class="mb-3">
                  <h4 class="text-sm font-bold text-gray-700 mb-2">التعليقات:</h4>
                  <div v-for="(comment, index) in task.comments" :key="index"
                    class="bg-gray-50 p-2 rounded-lg mb-1 text-xs">
                    <p>{{ comment.text }}</p>
                ...
                <div class="flex gap-2">
                  <input v-model="newComments[task.id]" @keyup.enter="addComment(task.id)" placeholder="أضف تعليقاً..."
                    class="flex-1 p-2 border rounded-lg text-sm" />
                  <button @click="addComment(task.id)"
                    class="bg-blue-500 text-white px-3 py-2 rounded-lg text-sm">إرسال</button>
                </div>
              </div>
            ...
            <div class="flex flex-col gap-2 justify-center min-w-[150px]">
              <button v-if="task.status === 'pending'" @click="updateStatus(task.id, 'working')"
                class="workflow-btn bg-blue-600 text-white">بدء العمل الآن</button>
              <button v-if="task.status === 'working'" @click="updateStatus(task.id, 'review')"
                class="workflow-btn bg-purple-600 text-white">تسليم المهمة</button>
              <button v-if="userRole === 'admin' && task.status === 'review'"
                @click="updateStatus(task.id, 'completed')" class="workflow-btn bg-green-600 text-white">اعتماد
                وإغلاق</button>

              <button v-if="userRole === 'admin'" @click="deleteTask(task.id)"
                class="text-red-400 text-xs font-bold mt-2 hover:underline">حذف المهمة 🗑️</button>
            </div>
          </div>
        </div>
      </div>
